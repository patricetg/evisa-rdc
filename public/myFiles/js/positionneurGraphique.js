...
function roundNumber(number, nbDecimal = 3)
{
    return parseFloat(parseFloat(number).toFixed(nbDecimal));
}
...
*/
        ...
        let offsetX = roundNumber(left,3); // Math.round(left) ; // e.offsetX;
        let offsetY = roundNumber(top,3); // Math.round(top); // e.offsetY;
